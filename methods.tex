...
the standard US quarter for comparison.
...
Table \ref{Tab:BatteryCompTable} shows the dfiferent current battery types available in the market (as seen in \cite{Web:BatteryTable}), and their parameters.
...
This was later changed to turn on everytime the data from the microcontroller was flushed to the memory chip,
an activity that would occur every 11.33 seconds.
The light would turn off after this process was complete,
taking roughly 43 milliseconds.
The result was a light that would blink every 11.33 seconds for 43 milliseconds.
\begin{figure}
\begin{center}
